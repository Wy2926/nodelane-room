...
def main():
    ...
    expected = {f"nodelane-room-{version}-{system}-{arch}{extension}"
    ...
    require(set(manifest) == expected, "expected exactly four release archives")
    ...
    for target,artifact in install_manifest['artifacts'].items():
        ...
        with tarfile.open(native/artifact['file']) as tf:
            ...
            require(tf.extractfile(node).read()==(release/f'nodelane-room-{version}-linux-{arch}'/'nlroom-node').read_bytes(),'native payload differs from full release')
    ...
    for name, digest in manifest.items():
        ...
        if name.endswith(".zip"):
            ...
            require("FROM golang" not in files["Dockerfile"].decode(), "release requires Go source")
            for entry in native.iterdir():
