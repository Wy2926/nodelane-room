...
端口允许 TCP/UDP 1–65535，规则数量及覆盖端口数量不设上限，不可重复；用起止区间表示完整范围，避免逐端口填写。诊断不占用游戏 UDP 4243。游戏规则和成员在同一事务快照内下发，成员授权到心跳后 45 秒，不生成逐成员/逐端口登记记录。配置版本冲突须刷新后重新编辑；更改会通知已有房间，客户端 Stop/Wait 后重启数据面。停用禁止新建和加入该游戏房间，并收回现有游戏流量授权。

未收录游戏可选“通用游戏”（`custom`），默认开放完整 TCP/UDP 范围与广播/组播，管理员可修改或停用，与其他游戏遵守相同规则。客户端仅选择游戏，不编辑网络权限。旧的 `room port`、`port/remove-port` RPC、`/endpoints` API 和旧端口状态字段已移除。

## Ethernet LAN 客户端准备

管理台“游戏管理”可配置广播、组播；额外 Ethernet 类型只在老游戏需要时填写，例如 IPX 的 `0x8137` 和 IEEE 802.3 的 `0`。动态端口游戏可配置 TCP、UDP 各一条 `1–65535`。所有房间均要求 `network.version: 1`；玩家须使用 `lan_version: 1`、本机 IPC 版本 2 的当前客户端，缺少策略或旧协议直接拒绝，不回退 L3。
...
游戏网卡固定 MTU **1500**，内部按 **1024 字节**分片，最大 Nebula 内层数据包 **1104 字节**；无需玩家手调 MTU。选型、丢包取舍与各协议边界统一见 [MTU 评审](docs/game-network.md#mtu-评审)。
