...
- PostgreSQL 为共享状态源；容量、地址、邀请、撤销、幂等与事件持久化须事务一致，SSE 断线可恢复快照，不依赖进程锁保证多副本正确性。数据库结构版本为 3（API 仍为 /v2）；仅初始化空库或打开当前结构，不迁移旧数据、不自动补表、不清库。
- 游戏统一使用已登记 TCP/UDP 端口和虚拟 IP 直连，不实现游戏专用发现或代理；通用单播、广播与组播 LAN 发现按 `docs/game-network.md` 分期验证；已配置游戏的端口由服务端管理，仅通用游戏允许客户端自定义。
