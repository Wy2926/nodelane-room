...
当前源码的 Ethernet LAN 架构使用数据库结构版本 6、API /v2。旧结构拒绝打开，不做迁移或清空；新部署使用独立空库。控制面为 0.3.1、客户端为 0.3.0、节点为 0.2.1；[镜像清单](../deploy/IMAGES.txt) 记录镜像摘要。历史 0.2.0 镜像不包含该架构，不能与当前客户端混用。

## 1Panel 管理现有容器

在“容器 → 编排”管理控制面和节点项目，编排引用实际持久卷/身份目录及固定镜像摘要；更新后重建对应服务并检查健康状态。已有 Compose 项目可用“导入”登记，不能因“应用商店”未列出就重复部署。

PostgreSQL 使用 1Panel 已安装的数据库容器；控制面编排加入其共享外部 Docker 网络，以容器名连接。数据库在 SQL 中创建后，在“数据库 → PostgreSQL → 从服务器同步”导入面板记录。同步已有库不会自动绑定既有数据库用户；不要为显示密码而创建多余超级用户或重新初始化业务库。

控制面私有状态卷与数据库均需保留。更新公开根以只读文件挂载，存储凭据加密密钥放在 root 可读的 `protected.env`，通过 Compose `env_file` 引用；所有连接同一数据库的实例使用同一密钥。更新配置与签名维护见 [客户端更新](updates.md)。
...
签名根、存储凭据加密、多源切换、发布及强制规则见 [客户端更新](updates.md)。当前 Compose 配置接受更新环境变量，公开根文件需通过本地覆盖文件只读挂载；不要把发布私钥放入控制容器。
...
当前只提供本站账号管理；IdP 的注册、密码与 MFA 在 IdP 管理。没有支付、跨账号自动合并、Back-Channel Logout 或用户供应同步，IdP 停用账号后需要在本站撤销全部设备以立即撤销连接，永久拒绝该身份须删除账号；“限制创建房间”保留登录、入房和已有连接。数据库结构为 6，只接受空库或当前结构，不迁移、不自动清理旧库；旧设备账号实现已替换，发布时统一交付匹配的客户端与服务端。
