...
CLI 提供 `--json`、`--watch`、退出码和房间子命令，作为开发与诊断工具维护。完整桌面包支持保留身份的手动更新，不提供旧命令别名或旧身份迁移流程。
...
统一 Ethernet LAN 的单播、广播/组播组件已实现，游戏本体兼容仍须 [验收](game-network.md#能力与验收边界)，收录资料不代表已验收。多控制端切换、玩家账号、游戏安装/启动、公共大厅和自动更新不在当前范围。
...
诊断按连接概览、虚拟 IP／授权、系统检查和成员链路呈现；系统报告标注检查时间，逐成员延迟与丢包仅在当前快照和授权有效时展示，未知值不填零。原始 JSON 不进入界面；复制使用字段白名单生成当前界面语言的脱敏摘要。设置按桌面偏好、设备信息、版本与更新、退出与联机分类；服务不可用时仍可打开设置和诊断。检查更新目前仅为界面占位，点击明确提示尚未接入在线服务，不请求发布源、不下载、不声称已是最新版。
...
扩展前先做固定 Nebula 与系统网络入口的小型集成验证；依赖变更另行评审与授权，不预建传输 Provider。在线签名更新及其他平台按实际需求推进，不承诺完成日期或移动端游戏兼容。
