...
无边框窗口保留系统窗口按钮、拖动区域与托盘行为。`preview.html?state=restricted/room/setup/error` 为独立开发预览；省略 state 展示可建房状态，`lang=en-US` 可检查英文。预览标明示例数据，使用公开 Steam 封面，不建立隧道或访问控制端，不进入正式构建。
