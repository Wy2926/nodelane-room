...
当前源码为访客/OIDC 账号、默认 4 人房间、统一 Ethernet LAN、客户端更新、数据库结构 6、本机 IPC 3、interaction-1 契约；控制面源码版本 0.3.1、客户端 0.3.0、节点 0.2.1。2026-09-12 已发布控制面 0.3.1 与节点 0.2.1 双架构镜像，以及 Windows/Linux amd64 的 0.3.0 完整客户端，摘要见 [镜像清单](../deploy/IMAGES.txt)及下方发布记录。下表保留原检查结果，后续改动不能据此自动判定通过；网络设计见 [游戏网络](game-network.md)。

| 对象 | 适用范围 |
|---|---|
| 当前 LAN 源码 | 单播、广播/组播、帧授权和分片已实现；组件和 Linux TAP 检查见下节，游戏本体仍未验收 |
| 当前完整安装包 | Windows/Linux amd64 0.3.0 已重建并经 TUF 签名发布，包含固定公开根；Windows 尚无 Authenticode 发布者签名，完整安装与驱动加载仍待真机验收 |
| 历史 0.2.0 安装包与镜像 | 使用旧数据面，不能与当前客户端混用；发布源码提交和镜像摘要见 [IMAGES.txt](../deploy/IMAGES.txt)，旧包/WebView 结果只列于历史证据 |

## 1Panel 与 R2 发布（2026-09-12）

源码基线 `7035c65` 加发布准备改动：控制面升为 0.3.1，固定初始公开更新根，并补齐 Linux Go 归档中 `nlroom-update` 的 0755 执行权限。控制面及节点镜像推送到 `docker.nodelane.net`；两个 1Panel 编排使用固定摘要部署，PostgreSQL 使用原 1Panel 容器内的结构 6 数据库并同步面板记录。

- Node.js 24.21.0：管理台 32 项、桌面 69 项测试及生产构建通过。Windows Go：gofmt、vet、全量测试及 OpenAPI/索引检查通过；未配置 Windows 测试数据库。此次仅版本与打包修正，未重跑 Linux 全量数据库/race；基线实际结果见下节控制面重构记录。
- Windows/Linux amd64 完整包构建通过。打包检查在 Windows 为 6 通过、7 个 Linux 用例跳过，Linux 为 13 通过。全组件 8 份归档及两个原生节点安装归档的 SHA256、架构、权限与内容检查通过。服务和更新助手的 8 个跨平台程序均包含同一个初始公开根。
- 最终两个镜像的 amd64/arm64 共 4 个程序与发布归档逐文件 SHA256 一致，远端 OCI 双架构摘要匹配。隔离镜像部署覆盖初始化、节点登记、TUN/身份重建恢复、UDP 配置、生命周期、控制面重建和撤销；[日志](../.local/redeploy-20260912/deploy-smoke-final.log)。ARM64 使用仿真检查，不代表真机验收。
- 线上服务和节点健康；中文/英文官网下载可用。R2 使用现有 `nodelane-room` 桶的专用对象读写凭据，凭据在数据库中加密。两份完整包均上传并由服务端核对 SHA256，已发布并设为普通推荐，不设置强制最低版本。
- 两份包均经公网 HTTPS 实际下载，大小与 SHA256 一致；真实 `/v2/updates/check` 返回的 TUF 元数据及下载文件通过客户端验证代码。Windows curl 的证书吊销服务暂不可达，实际下载改用保留默认 TLS 证书/主机名验证的 Python HTTPS 客户端；[结果](../.local/redeploy-20260912/public-verification.log)。

| 完整包 | SHA256 |
|---|---|
| `nlroom-0.3.0-windows-amd64-setup.exe`（21,995,527 字节） | `e5da800187bbc7e3b1fc5c03fdc957adb4df9b711c7ce67dd06619a3c5e0a2ad` |
| `nlroom_0.3.0_amd64.deb`（22,390,472 字节） | `8cd36e729a09249492765414933c763f9bc172231dba8a21a33f596a7e81e504` |

本次 TUF timestamp 到期时间为 **2026-10-12 13:26:50（北京时间）**，到期前需续签并重新导入清单，操作见 [更新发布](updates.md#发布与信任)。初始公开根 SHA256 为 `cf5947acf468c23e797938cc7cfd918f1c9e7443487110242f09fc45c076fee5`。私钥保存在本机隔离签名卷，未上传生产服务、R2 或 Git。Windows Authenticode、UAC/服务/驱动完整安装、双机 NAT/Minecraft 和 ARM 真机均未验收。

## 控制面审查与重构（2026-09-12）

基于 `83ae23b`：下载列表和更新总览改为只读一致性快照，发布及来源批量查询；下载地址保留写事务复查。控制面停机超时关闭剩余连接，协议拒绝响应纳入请求统计和禁止缓存处理。官网、管理台及此次重构尚未重新构建发布镜像。

- Windows Go 1.27.0：gofmt、`go vet ./...`、`go test -count=1 ./...` 通过，含 HTTP 正常排空、长连接超时关闭和拒绝响应回归；Windows 未配置测试数据库。[全量测试](../.local/repo-review/final-test.log)
- Linux Go 1.26.8、独立 PostgreSQL 18.6：vet、全量普通测试及 `go test -race -count=1 ./...` 通过，普通与 race 各 312 项含子用例通过、3 项跳过，数据库用例实际运行。覆盖持有业务写锁时的列表读取、下载地址等待变更与暂停后拒绝，以及真实 Nebula Ethernet、隔离/撤销、P2P 和中继。[race](../.local/repo-review/final-linux-race.log)、[结果与清理](../.local/repo-review/final-linux-results.json)
- 提交基线使用 Node.js 24.21.0：管理台 32 项、桌面 69 项测试及两端生产构建通过；后续重构未修改前端。OpenAPI、文件索引、包边界及差异检查通过。

Steam 在线导入与两项 GeoIP 样本测试因外部条件缺失跳过。本轮未运行 Docker 双客户端 TAP 拓扑、真实云存储/公网下载、Windows 服务/驱动与双机 NAT/Minecraft 验收；未部署或发布安装包。独立测试数据库、容器及网络已清理。
...
- Node.js 24.21.0 下管理台 14 项和桌面 69 项测试及构建通过；Go 格式、vet、全量测试、契约及索引通过。Linux 独立数据库全量与 race 结果见同一源码的客户端更新验证记录；Windows 本轮数据库用例未配置环境而跳过。
- 控制面、节点及同源节点安装归档的 SHA256、架构、内容和执行权限检查通过。全量 `check-release.py dist` 在非本次构建的旧客户端归档清单处失败（`unexpected component release archives: client`），不计为全量通过；客户端目录未参与镜像构建。
- 两组件 × 两架构的镜像程序版本和 SHA256 与发布包一致；ARM64 为 Docker 仿真运行版本命令，不代表 ARM 真机验收。
- 本次镜像运行隔离 Compose 冒烟：HTTPS/数据库/CA 初始化、节点登记、原生资源、TUN 重建、UDP 配置确认、登记标记恢复、生命周期操作、控制面重建及永久撤销均通过。[结果及清理](../.local/nodelane-deploy-test-44be8435cc2c/results.json)，[远端摘要核对](../.local/release-20260912/verified.json)。未运行本轮十分钟续签扩展。

初次构建使用系统 Node.js 22，已改用固定 Node.js 24 重新构建最终发布包。未更新线上容器、安装宿主服务、发布 latest 或桌面安装包；真实 UAC/安装器、双机 NAT/Minecraft 验收范围不变。
...
2026-09-12 补充启动更新弹窗：进入 GUI 主动检查；取消当前提示、持久跳过版本、下载字节进度及采样速度、取消 HTTP 请求、验证完成后单次自动安装已实现。强制版本不可跳过；安装器开始后不强行中断。

- 桌面构建和 69 项测试通过，覆盖跳过持久化、强制更新、下载取消后的迟到 ready 状态不安装，以及已验证包的一次点击安装；浏览器中英文预览通过，预览下载为明确的示例数据。
- Windows `go vet ./...`、`go test ./...` 通过，本机未配置数据库环境变量，数据库用例由下述 Linux 独立环境实际执行。Linux 独立 PostgreSQL 的 vet、全量普通测试及 `go test -race -count=1 ./...` 通过，包含真实请求取消；[本轮结果](../.local/nodelane-test-20260912-110432-b182ae/results.json)。隔离容器、卷、网络和本轮镜像已清理。
- Windows Rust 6 项通过，需已安装服务的 1 项跳过；契约生成、索引和包边界检查通过。尚未运行本轮 Windows UAC/EXE 或 Linux systemd/deb 真机更新，未生成或发布安装包。
