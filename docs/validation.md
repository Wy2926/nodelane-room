...
## 统一 Ethernet LAN 与旧实现移除（2026-09-11）

本轮将玩家数据面统一为 TAP + 固定 Nebula，移除旧 L3 游戏模式、端口登记表与 HTTP/IPC/CLI、客户端端口状态和 Wintun 分发。所有游戏由后端配置，TCP/UDP 可授权各自的完整端口范围；数据库结构为 4、本机 IPC 为 2，拒绝旧数据库，不提供迁移。MTU 选型与能力边界单独维护于 [游戏网络](game-network.md)。下方历史 0.2.0 产物与旧 TUN 验收不代表当前源码。

| 检查 | 实际结果 |
|---|---|
| Windows Go | gofmt、`go vet ./...`、`go test -count=1 ./...` 通过；包含真实 Nebula 进程内集成、LAN 分片/重组、端口授权、源地址/房间隔离、撤销、滑动去重窗口和契约检查。[vet](../.local/lan-windows-vet.log)、[测试](../.local/lan-windows-test.log) |
| Linux Go 与独立 PostgreSQL | `go vet ./...`、`go test -count=1 ./...` 和 `go test -race -count=1 ./...` 全部通过；数据库用例实际执行，包括结构 4、拒绝旧库、MAC 绑定、后端完整端口区间、策略替换与旧接口移除。[普通测试](../.local/nodelane-test-20260911-111443-488854/test.log)、[race](../.local/nodelane-test-20260911-111443-488854/race.log) |
| 真实 Linux TAP/Nebula | 隔离双客户端经真实原生 relay，通过 TCP、IPv4/IPv6 UDP、32 KB 数据报、有限/子网广播、组播、游戏 UDP 4243 与诊断共存、动态策略收紧、跨房拒绝和离房/关房/踢人撤销。测试容器 IPv4 底层 MTU 1280，TAP MTU 1500；不代表运营商 NAT 或 IPv6 公网测试。[运行记录](../.local/lan-final-verification.log) |
| 前端与桥接 | 桌面 TypeScript/Vite 构建和 24 项测试、管理台构建和 11 项测试通过。Windows Rust `cargo test --locked --offline` 两项通过，实际已安装服务的烟测跳过；已验证删除的端口操作被拒绝。[Rust](../.local/lan-rust-test.log) |
| 安装脚本与打包检查 | Windows 临时目录和模拟 SCM 下，安装事务 6 项、卸载 8 项通过；Python 打包检查 4 项通过、6 项 Linux 专用测试按平台跳过。当前未生成或验收新的完整安装包。[安装](../.local/lan-installer-test.log)、[卸载](../.local/lan-uninstall-test.log) |

真实 Nebula 进程内集成还使用三成员内存 TAP 验证 1514 字节帧、广播/组播复制、游戏 UDP 4243 与内存诊断并存、在线策略收紧及到期关闭。模拟帧包括 IPX/LLC，但不代表旧游戏协议栈已经真机验收。

回归中观察到诊断探测超时；有限次数探测逐次保存成功 RTT 与超时，不以一次成功声称零丢包。最终一轮 12 次主动探测均回复，但采集到的两端统计窗口仍包含约 8.33% 和 7.69% 的探测失败；原因与长期稳定性尚未验证。策略收紧测试先确认两端实际应用新游戏 revision，再检查旧端口和广播；仅检查保留端口连通不能证明更新已经生效。最终功能及源码检查全部通过，测试容器、私有卷、网络与本轮镜像已清理。[完整结果](../.local/nodelane-test-20260911-111443-488854/results.json)

Windows 未配置独立测试数据库，数据库验证由上述 Linux 环境执行。未运行实际 Windows TAP 驱动/服务、双机不同 NAT、游戏本体、IPv6 公网底层、长期吞吐/丢包/广播压力、ARM 真机及本轮完整原生 WebView 验收；WebView 脚本仅作语法检查。Linux 回归未启用外网 Steam 与 MMDB 样本测试。未安装宿主驱动或服务、修改宿主防火墙或发布。

## 无边框客户端与诊断可视化（2026-09-11）

客户端移除底栏并将导航与窗口控制集中至顶部，重排首页、游戏库和分类设置，提高正文与辅助信息字号；网络诊断使用真实状态和系统检查卡片，复制为字段白名单生成的脱敏摘要。检查更新按本轮范围仅做界面占位。Go 数据面、网络授权、HTTP／IPC 协议和依赖未变，原生能力仅增加主窗口拖动、最大化、最小化与关闭命令。

- TypeScript／Vite 构建、24 项前端测试通过，覆盖初始化前／服务离线时的系统入口、更新占位、诊断零值与未测量、快照陈旧、授权到期及脱敏复制；中途发现的图标导出和旧入口测试已修复。
- 浏览器预览检查默认 1120×760、最小 760×560 与窄屏 390×844 的布局，操作游戏选择／建房弹窗、设置分类／检查更新和诊断。首页与最小游戏库主操作可见，较长页面纵向滚动，顶部导航持续可达；最终加载后的控制台无新增 warning/error。预览使用明确标识的示例数据，不代表真实隧道或原生窗口验收。
- gofmt 检查、Windows `go vet ./...` 和 `go test ./...` 通过；另以 `go test -count=1 -v ./internal/engine` 重跑三项真实 Nebula UDP／握手／隔离／撤销／中继／P2P 集成测试，全部通过。该集成使用上游 UserDevice，不测试 Wintun 或公网 NAT。
- Windows Rust `cargo test --locked` 两项通过，`installed_service_roundtrip` 因未配置实际服务而跳过。原生 release GUI 构建通过，产物为 `dist/desktop/nlroom-windows-amd64-frameless.exe`，附 SHA256；仅 GUI、未签名，需已安装的同版 0.2.0 后台。Linux WebView 验收脚本已同步新的诊断与设置入口，仅完成 Python 语法检查，本轮未重跑完整 WebView 流程。

未配置独立 `NODELANE_TEST_DATABASE_URL`，数据库用例跳过；本轮无 Go 并发／控制状态改动，未重跑 Linux 数据库与 race。无边框原生拖动／缩放／托盘操作、Windows 服务和驱动、Linux 原生桌面、ARM64、双机 NAT／游戏本体仍须真机验收；浏览器预览不能替代。未安装宿主服务、修改防火墙或发布。
...
- 通用单播、广播/组播 LAN 已实现，组件与真实 Linux TAP 的检查见本页最新记录；游戏内列表发现仍须 [真机验收](game-network.md)。已删除的 Minecraft 专用发现/代理测试不计入当前通过范围。
