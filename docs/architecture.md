...
每份成员证书只有 `room:<id>` 组，基础设施只有 `infrastructure` 组。默认拒绝所有未列明端口；同房只允许 ICMP、UDP 4243 探测及已登记端口。终端入站只开放自己登记的端口。房间跨组默认拒绝；服务端按游戏配置校验端口，通用游戏允许成员自行登记本机端口。

每个叶证书最多十分钟，并受房间/CA 截止时间约束。退出/踢人/关房在事务中撤销该范围所有证书、删除端点和成员授权；客户端与节点快照带仍有效的撤销指纹，Nebula blocklist 拒绝旧证书重新握手，封装主动关闭已有隧道。撤销传播依赖控制连接；失联对端不可能即时得知撤销，因此安全上界是当前证书剩余有效期。
...
服务端在成员加入、心跳和管理员修改配置的同一事务里同步固定端口；端点有效期不超过成员最近心跳后 45 秒。配置变更清理旧端点并递增相关房间和管理事件版本。房间通过外键关联游戏，端口登记和配置更新在事务中保证授权，不在读取快照时补偿旧数据。停用禁止新建/加入和注册端口，旧房间仅保留控制与诊断；`custom` 始终可用且仅成员本人能增删自己的端口。

客户端只选择游戏，不自行覆盖已配置端口。当前数据面不区分游戏；预置 Minecraft 与 Steam 游戏遵守相同规则。游戏专用发现和代理已移除，通用 LAN 发现目标、转发边界及分期见 [游戏网络规划](game-network.md)。操作与导入来源说明统一在 README 维护。
