...
cmd/ 可执行程序入口
  ...
  nodelane-server/ Linux 控制面命令
    main.go CA、数据库初始化、管理员与 HTTP 服务
deploy/ 部署模板与测试环境
  .env.example 全套控制面配置示例
  .env.host.example 复用数据库与反代的配置示例
  .env.node.example 独立节点配置示例
  Caddyfile HTTPS 与双控制副本反代
  ...
  compose.yaml PostgreSQL、双控制副本与 Caddy 编排
...
internal/ 产品内部实现
  ...
  control/ 按调用方分文件的 HTTP API 与共享事务状态
    ...
    admin_auth.go 管理员密码、初始化、登录与会话事务
    ...
    admin_session_http.go 管理员初始化、登录、会话与 Cookie/CSRF 校验
    admin_snapshot.go 管理台总览与房间详情的一致性快照
    admin_test.go 管理会话、CSRF、节点路由、幂等、审计与事件恢复测试
    admin_web.go 管理页面、首页跳转与静态资源白名单
    adminweb/ 独立的管理台静态资源
      app.js 登录、节点与房间管理交互
      index.html 管理台页面结构
      style.css 管理台样式
  ...
  engine/ Nebula 数据面薄封装
    ...
    nebula_integration_test.go 真实 Nebula 隔离、重载、撤销与中继测试
