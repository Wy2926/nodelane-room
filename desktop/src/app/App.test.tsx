...
test("console shelf reaches games beyond eight by keyboard and search resets to a valid selection", async () => {
  const games = Array.from({ length: 10 }, (_, index) => ({ ...game, id: `game-${index}`, name: `冒险 ${index}` }));
  vi.mocked(rpc).mockImplementation(async (request) => (request.action === "games" ? games : []) as never);
  render(<App />);
  ...
  await user.type(screen.getByRole("searchbox", { name: "搜索游戏" }), "冒险 4");
  expect(screen.getByRole("button", { name: "冒险 4" }).getAttribute("aria-pressed")).toBe("true");
...
function joinedParty() {
  status.selected_room = "room";
  status.room = { id: "room", name: "周末世界", owner_id: "owner", game: game.id, game_name: game.name, revision: 1, capacity: 8, closed: false, expires_at: new Date(Date.now() + 3600000).toISOString() };
  status.game = game;
  status.control = "connected";
  status.engine = "running";
  status.members = [
    { device_id: "owner", name: "玩家", ip: "10.203.0.2", last_seen: new Date().toISOString() },
    { device_id: "guest", name: "远山", ip: "10.203.0.3", last_seen: new Date().toISOString() },
  ];
}
...
test("party management disclosure supports Escape and retains confirmation before transfer", async () => {
  ...
  expect(vi.mocked(rpc).mock.calls.some(([r]) => r.action === "transfer")).toBe(false);
  await user.click(screen.getByRole("button", { name: "关闭对话框" }));
  expect(document.activeElement).toBe(screen.getByRole("button", { name: "转让房主" }));
  await user.keyboard("{Escape}");
  expect(manage.closest("details")!.open).toBe(false);
  expect(document.activeElement).toBe(manage);
  await user.click(manage);
  ...
  expect(rpc).toHaveBeenCalledWith({ action: "transfer", room: "room", body: { device_id: "guest" } });
});

test.each([false, true])("member cards keep actual zero measurements and reject stale actions: stale=%s", async (stale) => {
  joinedParty();
  status.peers = [{ device_id: "guest", name: "远山", ip: "10.203.0.3", mode: "direct", rtt_ms: 0, loss_percent: 0 }];
  if (stale) status.snapshot_at = new Date(Date.now() - 60000).toISOString();
  render(<App />);
  expect(screen.getByText(stale ? "链路未知" : "直连")).toBeTruthy();
  expect(screen.getByText(stale ? "暂无实测数据" : "0.0 ms · 0% 丢包")).toBeTruthy();
  await userEvent.click(screen.getByLabelText("管理 远山"));
  expect((screen.getByRole("button", { name: "转让房主" }) as HTMLButtonElement).disabled).toBe(stale);
  expect((screen.getByRole("button", { name: "踢出成员" }) as HTMLButtonElement).disabled).toBe(stale);
  expect((screen.getByRole("button", { name: "测延迟" }) as HTMLButtonElement).disabled).toBe(stale);
});
...
test("offline startup keeps settings and the update placeholder reachable without network requests", async () => {
  ...
  expect(screen.getByRole("status").textContent).toContain("在线更新服务尚未接入");
  ...
  expect((screen.getByRole("button", { name: "运行诊断" }) as HTMLButtonElement).disabled).toBe(true);
});

test("diagnostics visualizes the real report and copies only an explicit redacted summary", async () => {
  vi.mocked(rpc).mockImplementation(async (request) => {
    if (request.action === "doctor") return {
      control: "connected", engine: "stopped", nebula_version: "1.11.1", device_id: "private-id", error: "private-error",
      virtual_ip: "10.203.0.7", unexpected_secret: "private-value", peers: [{ name: "private-peer" }],
      platform: { os: "windows", arch: "amd64", wintun_present: false, wintun_path: "private-path", interfaces: [{ name: "private-interface", up: false, mtu: 1500, addresses: ["192.0.2.9/24"] }] },
    } as never;
    return [] as never;
  });
  render(<App />);
...
test.each(["fresh", "stale", "expired", "offline", "unlinked", "unmeasured"])("diagnostic peer metrics respect actual measurements: %s", async (state) => {
  joinedParty();
  status.lease_expires_at = new Date(Date.now() + (state === "expired" ? -1000 : 60000)).toISOString();
  status.ip = "10.203.0.2";
  status.peers = [{ device_id: "guest", name: "远山", ip: "10.203.0.3", mode: state === "unlinked" ? "unknown" : "direct", rtt_ms: state === "unmeasured" ? undefined : 0, loss_percent: state === "unmeasured" ? undefined : 0 }];
  if (state === "stale") status.snapshot_at = new Date(Date.now() - 60000).toISOString();
  if (state === "offline") vi.mocked(useService).mockReturnValue({ status, error: { code: "service_unavailable", error: "服务离线" }, refresh: vi.fn(), updatedAt: Date.now() });
  render(<App />);
  await userEvent.click(screen.getByRole("button", { name: "网络诊断" }));
  if (state === "fresh") {
  ...
  if (["stale", "expired", "offline"].includes(state)) expect((screen.getByRole("button", { name: "测量 远山 的延迟" }) as HTMLButtonElement).disabled).toBe(true);
});
